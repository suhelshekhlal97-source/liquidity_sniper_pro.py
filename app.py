import streamlit as st
import pandas as pd
import pandas_ta as ta
import plotly.graph_objects as go
from datetime import datetime, timezone, timedelta
import numpy as np
import time
import requests
import gc

# ==========================================
# 1. APP CONFIGURATION
# ==========================================
st.set_page_config(
    page_title="Liquidity Sweep Sniper | Binance Pro",
    page_icon="🎯",
    layout="wide"
)

st.title("🎯 Liquidity Sweep Sniper | Binance Live Data")

# Custom CSS
st.markdown("""
<style>
    .stMetric { background-color: #1E1E1E; border: 1px solid #333; padding: 8px; border-radius: 5px; }
    .stDataFrame { border: 1px solid #333; border-radius: 5px; }
    .block-container { padding-top: 1rem; }
</style>
""", unsafe_allow_html=True)

# ==========================================
# 2. SIDEBAR CONTROLS
# ==========================================
st.sidebar.header("⚙️ Binance Configuration")

# Exchange selection
exchange = st.sidebar.radio("Exchange Mode", ["Binance Spot", "Binance Futures"], index=1)
IS_FUTURES = exchange == "Binance Futures"

# Asset selection (Binance symbols)
asset_map = {
    "Bitcoin (BTC/USDT)": "BTCUSDT",
    "Ethereum (ETH/USDT)": "ETHUSDT",
    "Binance Coin (BNB/USDT)": "BNBUSDT",
    "Solana (SOL/USDT)": "SOLUSDT",
    "Cardano (ADA/USDT)": "ADAUSDT",
    "XRP (XRP/USDT)": "XRPUSDT",
    "Gold (PAXG/USDT)": "PAXGUSDT",
    "Euro (EUR/USDT)": "EURUSDT",
}

selected_asset = st.sidebar.selectbox("Select Asset", list(asset_map.keys()))
SYMBOL = asset_map[selected_asset]

# Data mode
data_mode = st.sidebar.radio("Data Mode", ["Historical Backtest", "Live (Recent Data)"], index=0)

# Historical settings
if data_mode == "Historical Backtest":
    days = st.sidebar.slider("Days of 5m Data", 7, 30, 20, help="Max 30 days for Binance API")
else:
    days = 1  # Live mode uses recent data

# Risk settings
STARTING_CAPITAL = st.sidebar.number_input("Starting Capital ($)", 1000, 100000, 10000, step=1000)
RISK_PER_TRADE = st.sidebar.slider("Risk per Trade (%)", 0.5, 5.0, 1.0, step=0.5) / 100.0
MAX_ATTEMPTS = st.sidebar.number_input("Max Re-Entry Attempts", 1, 3, 2)

# Partial profit targets
TP1_R = 3.0  # 50%
TP2_R = 5.0  # 40%
TP3_R = 12.0  # 10%

# Session filter
session_filter = st.sidebar.toggle("Enable Session Filter (LON/NY)", value=True)

# Performance limits
MAX_CANDLES = st.sidebar.slider("Max Candles to Process", 1000, 5000, 3000, help="Prevent memory issues")

# ==========================================
# 3. BINANCE DATA FETCHER
# ==========================================
@st.cache_data(ttl=60)  # Cache for 1 minute
def fetch_binance_data(symbol, days, is_futures=False):
    """
    Fetches OHLCV data from Binance public API
    No API key required for klines endpoint
    """
    try:
        base_url = "https://fapi.binance.com" if is_futures else "https://api.binance.com"
        
        # Calculate timestamps
        end_time = int(time.time() * 1000)
        start_time = end_time - (days * 24 * 60 * 60 * 1000)
        
        all_data = []
        limit = 1000
        
        while start_time < end_time:
            params = {
                "symbol": symbol,
                "interval": "5m",
                "startTime": start_time,
                "limit": limit
            }
            
            response = requests.get(f"{base_url}/api/v3/klines", params=params, timeout=10)
            response.raise_for_status()
            
            data = response.json()
            if not data:
                break
                
            all_data.extend(data)
            start_time = data[-1][6] + 1  # Next start from last close time
            
            # Rate limit safety
            time.sleep(0.05)
        
        # Convert to DataFrame
        df = pd.DataFrame(all_data, columns=[
            'timestamp', 'open', 'high', 'low', 'close', 'volume',
            'close_time', 'quote_volume', 'trades', 'taker_buy_base',
            'taker_buy_quote', 'ignore'
        ])
        
        df['timestamp'] = pd.to_datetime(df['timestamp'], unit='ms')
        df.set_index('timestamp', inplace=True)
        df = df[['open', 'high', 'low', 'close', 'volume']].astype(float)
        df.columns = ['Open', 'High', 'Low', 'Close', 'Volume']
        
        # Remove duplicates and sort
        df = df[~df.index.duplicated(keep='last')].sort_index()
        
        return df
        
    except requests.exceptions.RequestException as e:
        st.error(f"❌ Binance API Error: {e}")
        st.info("Attempting fallback to yfinance...")
        return fetch_yfinance_fallback(symbol, days)
    except Exception as e:
        st.error(f"❌ Unexpected error: {e}")
        return pd.DataFrame()

def fetch_yfinance_fallback(symbol, days):
    """Fallback to yfinance if Binance fails"""
    try:
        yf_symbol = symbol.replace('USDT', '-USD')
        import yfinance as yf
        data = yf.download(yf_symbol, period=f"{days}d", interval="5m", progress=False)
        if isinstance(data.columns, pd.MultiIndex):
            data.columns = data.columns.get_level_values(0)
        return data.dropna()
    except:
        st.error("❌ Both Binance and yfinance failed.")
        return pd.DataFrame()

# ==========================================
# 4. SETUP TRACKER & BACKTEST ENGINE
# ==========================================
class SetupTracker:
    """Tracks re-attempts per liquidity level"""
    def __init__(self):
        self.attempts = 0
        self.current_level = None
    
    def can_trade(self, level):
        if pd.isna(level):
            return False
        if level != self.current_level:
            self.current_level = level
            self.attempts = 0
        return self.attempts < MAX_ATTEMPTS
    
    def record_attempt(self):
        self.attempts += 1

def run_liquidity_sweep_backtest(dataframe, capital):
    df = dataframe.copy()
    trades = []
    setup_tracker = SetupTracker()
    
    # === INDICATORS ===
    df['EMA_1h'] = ta.ema(df['Close'], length=600)
    df['EMA_4h'] = ta.ema(df['Close'], length=2400)
    df['ATR'] = ta.atr(df['High'], df['Low'], df['Close'], length=14)
    
    # Liquidity levels (previous hour)
    df['Hour_Start'] = df.index.to_series().dt.floor('H')
    hourly_lows = df.groupby('Hour_Start')['Low'].min()
    hourly_highs = df.groupby('Hour_Start')['High'].max()
    df['Liquidity_Low'] = df['Hour_Start'].map(hourly_lows.shift(1))
    df['Liquidity_High'] = df['Hour_Start'].map(hourly_highs.shift(1))
    
    # Session filter
    df['Hour'] = df.index.hour
    df['Session'] = "ASIAN"
    df.loc[(df['Hour'] >= 7) & (df['Hour'] <= 16), 'Session'] = "LON"
    df.loc[(df['Hour'] >= 13) & (df['Hour'] <= 21), 'Session'] = "NY"
    df['In_Session'] = ~session_filter | (df['Session'].isin(["LON", "NY"]))
    
    # Previous candle
    df['Prev_Low'] = df['Low'].shift(1)
    df['Prev_High'] = df['High'].shift(1)
    
    # Limit processing to prevent memory issues
    df = df.tail(MAX_CANDLES)
    
    position = None
    
    for i in range(2, len(df) - 20):
        # Exit logic
        if position:
            curr_high = df['High'].iloc[i]
            curr_low = df['Low'].iloc[i]
            pnl = 0
            result = ""
            
            # TP1 (3R, 50%)
            if not position['tp1_hit']:
                if (position['type'] == 'LONG' and curr_high >= position['tp1']) or \
                   (position['type'] == 'SHORT' and curr_low <= position['tp1']):
                    pnl += (position['tp1'] - position['entry']) * position['size'] * 0.5 if position['type'] == 'LONG' else \
                           (position['entry'] - position['tp1']) * position['size'] * 0.5
                    position['tp1_hit'] = True
            
            # TP2 (5R, 40%) + Move SL to BE
            if position['tp1_hit'] and not position['tp2_hit']:
                if (position['type'] == 'LONG' and curr_high >= position['tp2']) or \
                   (position['type'] == 'SHORT' and curr_low <= position['tp2']):
                    pnl += (position['tp2'] - position['entry']) * position['size'] * 0.4 if position['type'] == 'LONG' else \
                           (position['entry'] - position['tp2']) * position['size'] * 0.4
                    position['tp2_hit'] = True
                    position['sl'] = position['entry']  # Breakeven
            
            # TP3 (12R, 10%)
            if position['tp2_hit'] and not position['tp3_hit']:
                if (position['type'] == 'LONG' and curr_high >= position['tp3']) or \
                   (position['type'] == 'SHORT' and curr_low <= position['tp3']):
                    pnl += (position['tp3'] - position['entry']) * position['size'] * 0.1 if position['type'] == 'LONG' else \
                           (position['entry'] - position['tp3']) * position['size'] * 0.1
                    position['tp3_hit'] = True
                    result = "TP3 HIT"
            
            # SL
            if not position['tp3_hit']:
                if (position['type'] == 'LONG' and curr_low <= position['sl']) or \
                   (position['type'] == 'SHORT' and curr_high >= position['sl']):
                    remaining_size = position['size']
                    if position['tp1_hit']:
                        remaining_size *= 0.5
                    if position['tp2_hit']:
                        remaining_size *= 0.6
                    
                    pnl += (position['sl'] - position['entry']) * remaining_size if position['type'] == 'LONG' else \
                           (position['entry'] - position['sl']) * remaining_size
                    result = "SL HIT"
            
            if result:
                capital += pnl
                trades.append({
                    'Setup_Time': position['setup_time'],
                    'Attempt': f"{position['attempt']}/{MAX_ATTEMPTS}",
                    'Type': position['type'],
                    'Entry': position['entry'],
                    'SL': position['sl'],
                    'TP1': position['tp1'],
                    'TP2': position['tp2'],
                    'TP3': position['tp3'],
                    'Exit_Price': position['sl'] if 'SL' in result else position['tp3'],
                    'Result': result,
                    'PnL': pnl,
                    'Capital': capital
                })
                position = None
                continue
        
        # Entry logic
        if position is None:
            liquidity_low = df['Liquidity_Low'].iloc[i]
            liquidity_high = df['Liquidity_High'].iloc[i]
            
            if not setup_tracker.can_trade(liquidity_low) and not setup_tracker.can_trade(liquidity_high):
                continue
            
            # Trend filter
            is_bullish = df['Close'].iloc[i] > df['EMA_1h'].iloc[i] and df['Close'].iloc[i] > df['EMA_4h'].iloc[i]
            is_bearish = df['Close'].iloc[i] < df['EMA_1h'].iloc[i] and df['Close'].iloc[i] < df['EMA_4h'].iloc[i]
            
            # Sweep detection
            swept_low = df['Low'].iloc[i] < liquidity_low
            reclaimed_bull = df['Close'].iloc[i] > liquidity_low
            prev_not_swept_bull = df['Prev_Low'].iloc[i] > liquidity_low
            
            swept_high = df['High'].iloc[i] > liquidity_high
            reclaimed_bear = df['Close'].iloc[i] < liquidity_high
            prev_not_swept_bear = df['Prev_High'].iloc[i] < liquidity_high
            
            # Execute
            if is_bullish and swept_low and reclaimed_bull and prev_not_swept_bull:
                entry_price = df['Open'].iloc[i+1]
                atr_val = df['ATR'].iloc[i]
                sl = df['Low'].iloc[i] - (atr_val * 0.1)
                risk_per_coin = entry_price - sl
                
                position_size = (capital * RISK_PER_TRADE) / risk_per_coin
                
                position = {
                    'type': 'LONG',
                    'entry': entry_price,
                    'sl': sl,
                    'tp1': entry_price + (risk_per_coin * TP1_R),
                    'tp2': entry_price + (risk_per_coin * TP2_R),
                    'tp3': entry_price + (risk_per_coin * TP3_R),
                    'size': position_size,
                    'setup_time': df.index[i+1],
                    'attempt': setup_tracker.attempts + 1,
                    'tp1_hit': False,
                    'tp2_hit': False,
                    'tp3_hit': False
                }
                setup_tracker.record_attempt()
            
            elif is_bearish and swept_high and reclaimed_bear and prev_not_swept_bear:
                entry_price = df['Open'].iloc[i+1]
                atr_val = df['ATR'].iloc[i]
                sl = df['High'].iloc[i] + (atr_val * 0.1)
                risk_per_coin = sl - entry_price
                
                position_size = (capital * RISK_PER_TRADE) / risk_per_coin
                
                position = {
                    'type': 'SHORT',
                    'entry': entry_price,
                    'sl': sl,
                    'tp1': entry_price - (risk_per_coin * TP1_R),
                    'tp2': entry_price - (risk_per_coin * TP2_R),
                    'tp3': entry_price - (risk_per_coin * TP3_R),
                    'size': position_size,
                    'setup_time': df.index[i+1],
                    'attempt': setup_tracker.attempts + 1,
                    'tp1_hit': False,
                    'tp2_hit': False,
                    'tp3_hit': False
                }
                setup_tracker.record_attempt()
    
    # Close open position at end
    if position:
        exit_price = df['Close'].iloc[-1]
        pnl = 0
        if position['tp1_hit']:
            pnl += (position['tp1'] - position['entry']) * position['size'] * 0.5 if position['type'] == 'LONG' else \
                   (position['entry'] - position['tp1']) * position['size'] * 0.5
        if position['tp2_hit']:
            pnl += (position['tp2'] - position['entry']) * position['size'] * 0.4 if position['type'] == 'LONG' else \
                   (position['entry'] - position['tp2']) * position['size'] * 0.4
        remaining_size = position['size']
        if position['tp1_hit']:
            remaining_size *= 0.5
        if position['tp2_hit']:
            remaining_size *= 0.6
        pnl += (exit_price - position['entry']) * remaining_size if position['type'] == 'LONG' else \
               (position['entry'] - exit_price) * remaining_size
        
        capital += pnl
        trades.append({
            'Setup_Time': position['setup_time'],
            'Attempt': f"{position['attempt']}/{MAX_ATTEMPTS}",
            'Type': position['type'],
            'Entry': position['entry'],
            'SL': position['sl'],
            'TP1': position['tp1'],
            'TP2': position['tp2'],
            'TP3': position['tp3'],
            'Exit_Price': exit_price,
            'Result': 'OPEN',
            'PnL': pnl,
            'Capital': capital
        })
    
    # Memory cleanup
    gc.collect()
    
    return pd.DataFrame(trades), capital

# ==========================================
# 5. RUN BACKTEST
# ==========================================
with st.spinner(f"Fetching {'live' if data_mode == 'Live (Recent Data)' else 'historical'} data from Binance for {SYMBOL}..."):
    df = fetch_binance_data(SYMBOL, days, IS_FUTURES)

if df.empty:
    st.error("❌ No data available. Check symbol, network, or Binance API status.")
    st.stop()

# Run backtest
with st.spinner("Running MTF Liquidity Sweep backtest..."):
    trade_history, final_capital = run_liquidity_sweep_backtest(df, STARTING_CAPITAL)

# ==========================================
# 6. DASHBOARD UI
# ==========================================
# Top metrics
current_price = df['Close'].iloc[-1]
change = current_price - df['Close'].iloc[-2]

col1, col2, col3, col4 = st.columns(4)
col1.metric("Live Price", f"${current_price:,.2f}", f"{change:.2f}")
col2.metric("Final Capital", f"${final_capital:,.2f}", f"{(final_capital-STARTING_CAPITAL)/STARTING_CAPITAL*100:.1f}%")
col3.metric("Total Setups", len(trade_history['Setup_Time'].unique()) if not trade_history.empty else 0)
col4.metric("Avg PnL/Setup", f"${trade_history.groupby('Setup_Time')['PnL'].sum().mean():.2f}" if not trade_history.empty else "$0.00")

# Chart
st.markdown("### 📊 5-Min Chart with Setups")
plot_df = df.tail(1000)

fig = go.Figure()
fig.add_trace(go.Candlestick(
    x=plot_df.index, open=plot_df['Open'], high=plot_df['High'],
    low=plot_df['Low'], close=plot_df['Close'], name="Price"
))
fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['EMA_1h'], line=dict(color='orange', width=1), name="50 EMA (1H)", opacity=0.7))
fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['EMA_4h'], line=dict(color='blue', width=1), name="50 EMA (4H)", opacity=0.7))

# Mark trades
if not trade_history.empty:
    recent_trades = trade_history[pd.to_datetime(trade_history['Setup_Time']) >= plot_df.index[0]]
    for _, trade in recent_trades.iterrows():
        color = "cyan" if 'LONG' in trade['Type'] else "magenta"
        fig.add_annotation(x=trade['Setup_Time'], y=trade['Entry'], text="▲", showarrow=False, 
                          yshift=15, font=dict(color=color, size=20))

fig.update_layout(height=500, template="plotly_dark", margin=dict(l=0,r=0,t=0,b=0),
                  xaxis_rangeslider_visible=False, showlegend=True)
st.plotly_chart(fig, use_container_width=True)

# Trade history
st.markdown("---")
st.subheader("📈 Trade History")

if not trade_history.empty:
    # Setup-level summary
    setup_summary = trade_history.groupby('Setup_Time').agg({
        'Attempt': lambda x: f"{len(x)}/{MAX_ATTEMPTS}",
        'Type': 'first',
        'Entry': 'first',
        'SL': 'first',
        'TP1': 'first',
        'TP2': 'first',
        'TP3': 'first',
        'PnL': 'sum',
        'Result': lambda x: 'RE-ENTRY WIN' if len(x) > 1 and 'TP3' in x.iloc[-1] else x.iloc[-1],
        'Capital': 'last'
    }).reset_index()
    
    st.dataframe(
        setup_summary.sort_values('Setup_Time', ascending=False),
        use_container_width=True,
        hide_index=True,
        column_config={
            "Entry": st.column_config.NumberColumn("Entry", format="%.2f"),
            "SL": st.column_config.NumberColumn("Stop Loss", format="%.2f"),
            "TP1": st.column_config.NumberColumn(f"TP1 ({TP1_R}R)", format="%.2f"),
            "TP2": st.column_config.NumberColumn(f"TP2 ({TP2_R}R)", format="%.2f"),
            "TP3": st.column_config.NumberColumn(f"TP3 ({TP3_R}R)", format="%.2f"),
            "PnL": st.column_config.NumberColumn("PnL ($)", format="$.2f"),
        }
    )
    
    with st.expander("View Detailed Trade Log"):
        st.dataframe(
            trade_history.sort_values('Setup_Time', ascending=False),
            use_container_width=True,
            hide_index=True
        )
else:
    st.warning("No trades found. Try adjusting parameters or expanding data range.")

# Strategy info
st.sidebar.markdown("---")
st.sidebar.subheader("📖 Strategy Logic")
st.sidebar.caption(f"""
**Exchange:** Binance {'Futures' if IS_FUTURES else 'Spot'}  
**Data Mode:** {data_mode}  
**Asset:** {selected_asset}  
**Trend:** 50 EMA on 1H & 4H (5m chart)  
**Liquidity:** Previous hour high/low  
**Entry:** Next candle after sweep + reclaim  
**Risk:** {RISK_PER_TRADE*100}% per attempt, {MAX_ATTEMPTS} max per setup  
**Targets:** {TP1_R}R (50%), {TP2_R}R (40%), {TP3_R}R (10%)  
**Session:** LON (07-16 UTC) + NY (13-21 UTC)  
**Buffer:** 10% of ATR beyond sweep extreme
""")

st.sidebar.caption(f"*Processed {len(df)} 5m candles*")

# Download results
if not trade_history.empty:
    csv = setup_summary.to_csv(index=False)
    st.sidebar.download_button(
        label="📥 Download Results (CSV)",
        data=csv,
        file_name=f"binance_sniper_{SYMBOL}_{datetime.now().strftime('%Y%m%d_%H%M')}.csv",
        mime="text/csv"
    )

# Manual refresh for live mode
if data_mode == "Live (Recent Data)":
    st.sidebar.info("🔄 Live mode - data refreshes every minute")
    if st.sidebar.button("Force Refresh"):
        st.rerun()

# Footer
st.sidebar.markdown("---")
st.sidebar.caption("Built with 💻 by Trading Algo Team")
